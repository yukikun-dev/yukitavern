...
export class FilterHelper {
    ...
    tagFilter(data) {
        ...
        function getIsTagged(entity) {
            const tagFlags = selected.map((tagId) =>
            ...
            const excludedTagFlags = excluded.map((tagId) =>
                isElementTagged(entity, tagId),
            );
    ...
    favFilter(data) {
        ...
        return data.filter(
            (entity) => entity.item.fav || entity.item.fav == "true",
        );
    }
    ...
    searchFilter(data) {
        ...
        const searchValue = this.filterData[FILTER_TYPES.SEARCH]
            .trim()
            .toLowerCase();
        const fuzzySearchCharactersResults = power_user.fuzzy_search
            ? fuzzySearchCharacters(searchValue)
            : [];
        const fuzzySearchGroupsResults = power_user.fuzzy_search
            ? fuzzySearchGroups(searchValue)
            : [];
        ...
        function getIsValidSearch(entity) {
            ...
            if (power_user.fuzzy_search) {
                if (isCharacter) {
                    return fuzzySearchCharactersResults.includes(
                        parseInt(entity.id),
                    );
                } else if (isGroup) {
                ...
                return (
                    entity.item?.name?.toLowerCase()?.includes(searchValue) ||
                    false
                );
            }
        }
    ...
    applyFilters(data) {
        return Object.values(this.filterFunctions).reduce(
            (data, fn) => fn(data),
            data,
        );
    }
