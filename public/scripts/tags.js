...
async function importTags(imported_char) {
    ...
    if (newTags.length === 0) {
        ...
        selected_tags = await callPopup(
            `<h3>Importing Tags For ${imported_char.name}</h3><p>${existingTags.length} existing tags have been found${existingTagsString}.</p><p>The following ${newTags.length} new tags will be imported.</p>`,
            "input",
            newTags.join(", "),
        );
    }
...
function printTagFilters(type = tag_filter_types.character) {
    ...
    for (const tag of Object.values(ACTIONABLE_TAGS)) {
        appendTagToList(FILTER_SELECTOR, tag, {
            removable: false,
            selectable: false,
            action: tag.action,
            isGeneralList: true,
