...
async function doDiceRoll(customDiceFormula) {
    let value =
        typeof customDiceFormula === "string"
            ? customDiceFormula.trim()
            : $(this).data("value");

    if (value == "custom") {
        value = await callPopup(
            "Enter the dice formula:<br><i>(for example, <tt>2d6</tt>)</i>",
            "input",
        );
    }
    ...
    if (isValid) {
        ...
        context.sendSystemMessage(
            "generic",
            `${context.name1} rolls a ${value}. The result is: ${result.total} (${result.rolls})`,
            { isSmallSys: true },
        );
    } else {
...
jQuery(function () {
    ...
    registerSlashCommand(
        "roll",
        (_, value) => doDiceRoll(value),
        ["r"],
        "<span class='monospace'>(dice formula)</span> – roll the dice. For example, /roll 2d6",
        false,
        true,
    );
});
