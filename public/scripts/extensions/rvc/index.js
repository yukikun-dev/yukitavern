...
async function rvcVoiceConversion(response, character) {
    ...
    if (extension_settings.rvc.voiceMap[character] === undefined) {
        toastr.error(
            "No model is assigned to character '" +
                character +
                "', check RVC voice map in the extension menu.",
            DEBUG_PREFIX + "RVC Voice map error",
            { timeOut: 10000, extendedTimeOut: 20000, preventDuplicates: true },
        );
        console.error(
            "No RVC model assign in voice map for current character " +
                character,
        );
        return response;
    }
    ...
    if (!apiResult.ok) {
        toastr.error(
            ...
            DEBUG_PREFIX + " RVC Voice Conversion Failed",
            { timeOut: 10000, extendedTimeOut: 20000, preventDuplicates: true },
        );
...
function loadSettings() {
    ...
    $("#rvc_pitch_extractiont_value").text(
        extension_settings.rvc.pitchExtraction,
    );
...
async function onApplyClick() {
    ...
    for (const text of array) {
        if (text.includes(":")) {
            ...
            console.debug(
                ...
                pair[0],
                "=>",
                extension_settings.rvc.voiceMap[pair[0]],
            );
        } else {
            $("#rvc_status").text(
                "Voice map is invalid, check console for errors",
            );
    ...
    if (!error) {
        ...
        toastr.info(
            "New map:\n" +
                JSON.stringify(extension_settings.rvc.voiceMap).substring(
                    0,
                    200,
                ) +
                "...",
