    ...
    for (const avatar of group.members) {
        ...
        if (expressionImage.length > 0) {
            if (name == spriteFolderName) {
                ...
                const currentSpritePath = labels.includes(expression)
                    ? sprites.find((x) => x.label === expression)?.path
                    : "";
...
function onExpressionsShowDefaultInput() {
    ...
    if (existingImageSrc !== undefined) {
        //if we have an image in src
        if (!value && existingImageSrc.includes("/img/default-expressions/")) {
            //and that image is from /img/ (default)
...
async function loadLiveChar() {
    ...
    try {
        ...
        const spriteFile = new File([spriteBlob], "talkinghead.png", {
            type: "image/png",
        });
